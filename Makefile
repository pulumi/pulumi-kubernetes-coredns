...
codegen: .pulumi/bin/pulumi # Required by CI
	mkdir -p bin && touch bin/pulumi-gen-kubernetes-coredns
...
sign-windows-exe-%: bin/jsign-6.0.jar
	...
	if [[ "${SKIP_SIGNING}" != "true" ]]; then \
		if [[ "|${AZURE_SIGNING_CLIENT_ID}|${AZURE_SIGNING_CLIENT_SECRET}|${AZURE_SIGNING_TENANT_ID}|${AZURE_SIGNING_KEY_VAULT_URI}|" == *"||"* ]]; then \
			...
			file=dist/pulumi-kubernetes-coredns_windows_${GORELEASER_ARCH}/pulumi-resource-kubernetes-coredns.exe; \
